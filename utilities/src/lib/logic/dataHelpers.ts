...
const insertAccountData = (accountScores: AccountScoreData, blockNumber: number, account: string, data: AccountScoreDataPoint) => {
...
export const convertScoreDataToPerAccount = (scoreData: ScoreDataHistory) => {
  ...
  const blocks = Object.keys(scoreData).map(e => parseInt(e)).sort();
