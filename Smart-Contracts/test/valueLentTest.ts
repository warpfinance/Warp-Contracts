...
contract("Control", function(accounts) {
  it("Calculates Total Value Lent", async () => {
    ...
    const expectedAmount = new BigNumber((await warpControl.viewPriceOfToken(daiToken.address, amountLentExpected.toString())).toString());

    const totalLent = await warpControl.viewTotalLentValue(testerAddress);
    console.log(totalLent.toString(), expectedAmount.toString());
    assert(totalLent.toString() == expectedAmount.toString());
